...
describe("createLocalApp", () => {
	it("serves a public GET route", async () => {
		const entry = mkFn('export const main = async () => ({ statusCode: 200, body: "hi" });');
	...
	it("calls the authorizer and 403s on deny", async () => {
		...
		const entry = mkFn('export const main = async () => ({ statusCode: 200, body: "private" });');
		const m: LocalManifest = {
			source: "cdk-synth",
			stack: "S",
			stage: "dev",
	...
	it("invokes onError and returns 500 when the handler throws", async () => {
		const entry = mkFn('export const main = async () => { throw new Error("boom"); };');
		const m: LocalManifest = {
			source: "cdk-synth",
			stack: "S",
			stage: "dev",
	...
	it("throws when a route references an unknown function", async () => {
		...
		await expect(createLocalApp({ manifest: m, watch: false })).rejects.toThrow(/unknown lambda/i);
	});

	it("prefers literal routes over parameterized routes (specificity)", async () => {
		const meEntry = mkFn('export const main = async () => ({ statusCode: 200, body: "me" });');
		const idEntry = mkFn(
