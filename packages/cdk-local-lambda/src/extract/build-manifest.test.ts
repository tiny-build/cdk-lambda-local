  ...
  const template = {
    Resources: {
      ...
      HelloFn: {
        ...
        Properties: {
          FunctionName: 'pat-services-dev-hello',
          Handler: 'index.main',
          Runtime: 'nodejs22.x',
      ...
      AuthFn: {
        Type: 'AWS::Lambda::Function',
        Properties: {
          FunctionName: 'pat-services-dev-authorizer',
          Handler: 'index.main',
          Runtime: 'nodejs22.x',
...
describe('extractManifest', () => {
  it('produces a v2 manifest with lambdas, route, and authorizerKey', async () => {
    ...
    expect(Object.keys(m.lambdas).sort()).toEqual(['authorizer', 'hello']);
    const hello = m.lambdas.hello!;
    expect(hello.handler).toBe('main');
    expect(hello.runtime).toBe('nodejs22.x');
    expect(hello.entry).toContain('api/src/functions/hello/handler.ts');
    expect(hello.environment.STAGE).toBe('dev');
    expect(hello.lambdaFunctionName).toBe('pat-services-dev-hello');
